...
function login() {
  ...
  var password = control.getInput("password");
  var otp = control.getInput("otp"); // Retrieve OTP value

  if (username == "") {
      control.popup("Enter Username", W);
  } else if (password == "") {
      control.popup("Enter Password", W);
  } else if (otp == "" && document.getElementById("otpField").style.display !== "none") {
      control.popup("Enter OTP", W);
  } else {
      control.html("lgbtn", "wait...");
      $.ajax({
          url: "../../assets/backend/auth/validate.php",
          type: "post",
          data: "username=" + username + "&password=" + password + "&otp=" + otp, // Include OTP in data
          success: function (htl) {
              var html = JSON.parse(htl);
              if (html.status != 1) {
                  control.popup(html.msg, W);
...
function sendOTP() {
  ...
  if (email == "") {
      ...
      $.ajax({
          url: "../../assets/backend/auth/send_otp.php", // Replace with your server-side script handling email and OTP sending
          type: "post",
          data: { email: email },
          success: function (response) {
              control.popup("OTP sent to your email", W);
          },
          error: function () {
              control.popup("Failed to send OTP", W);
          },
      });
  }
}
function mobile_sent_otp() {
  ...
  if (id != "") {
    ...
    $.ajax({
      ...
      success: function (html) {
        alert(html);
        swal("OTP Sent Successfuly", "", S).then(function () {
...
function email_sent_otp() {
  ...
  if (id != "") {
    control.html("email-sent_otp", "Sending mail wait..");
    $.ajax({
